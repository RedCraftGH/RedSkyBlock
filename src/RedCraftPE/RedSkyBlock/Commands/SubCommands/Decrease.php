      ...
      if (count($args) < 3) {
        ...
        if (is_numeric($args[1]) && intval($args[1]) > 0) {
          ...
          if ($player) {
            ...
            $player->sendMessage(TextFormat::GREEN . "Your island's limits have been decreased by {$amount}");
          }
          ...
          if (array_key_exists($name, $skyblockArray)) {
            ...
            $sender->sendMessage(TextFormat::WHITE . $name . "'s" . TextFormat::GREEN . " island limits have been decreased by {$amount}");
            return true;
          } else {
